...
object TokenAuthenticator {
  object TokenExtraction {
    ...
    def fromHeader(headerName: String): TokenExtractor = { context: RequestContext =>
      context.request.headers.find(_.lowercaseName == headerName.toLowerCase).map(_.value)
    }
  ...
  class TokenAuthenticator[T](extractor: TokenExtraction.TokenExtractor, authenticator: (String => Future[Option[T]]))(implicit ec: ExecutionContext) extends ContextAuthenticator[T] {
  ...
  def apply[T](headerName: String, queryStringParameterName: String)(authenticator: (String => Future[Option[T]]))(implicit ec: ExecutionContext) = {
